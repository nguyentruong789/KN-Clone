...
public class ExplodableEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        {
            EditorGUILayout.HelpBox("Use subshatter steps with caution! Too many will break performance!!! Don't recommend more than 1", UnityEditor.MessageType.Warning);
        }
        ...
        {
            EditorGUILayout.HelpBox("You must add a BoxCollider2D or PolygonCollider2D to explode this sprite", UnityEditor.MessageType.Warning);
        }
        else
        {
            ...
            if (GUILayout.Button("Set Fragments original position"))
            {
                myTarget.SetFragmentOriginalPosition();
                EditorUtility.SetDirty(myTarget);
            }
        }
